...
// ==========================================
// MOCK DATA: ESTRUCTURA DEL CASO DE ESTUDIO
// ==========================================
const projectData = {
  id: 1,
  title: "TenisDrop AI",
  ...
  overview: "TenisDrop AI revolucionó la forma en que los coleccionistas predicen el valor de reventa de zapatillas de edición limitada. El desafío principal era construir una interfaz capaz de renderizar miles de puntos de datos financieros en tiempo real, manteniendo una experiencia de usuario fluida y una latencia casi nula, todo esto empaquetado en un diseño premium y de alto rendimiento.",
  challenge: "El cliente contaba con un modelo de Machine Learning en Python extremadamente potente, pero carecía de una plataforma web que pudiera consumir esa API y mostrar los gráficos de predicción sin congelar el navegador. Además, necesitaban un sistema de autenticación robusto y una pasarela de pagos para el modelo SaaS.",
  solution: "Implementamos una arquitectura Headless utilizando Next.js para el SSR y SEO, conectado a un backend de Python/FastAPI. Utilizamos WebSockets para actualizar los precios en vivo y Canvas/WebGL para renderizar los gráficos financieros, logrando mantener los 60 FPS en dispositivos móviles.",
  techStack: [
    ...
    { name: "TensorFlow", icon: <Database size={20} /> }
  ],
  gallery: [
    ...
    "https://images.unsplash.com/photo-1557821552-17105176677c?q=80&w=1500&auto=format&fit=crop"
  ],
  results: [
...
const Navbar = ({ isDark, toggleTheme }) => {
  return (
    <nav className="fixed top-0 left-0 w-full z-50 px-6 py-6 bg-white/80 dark:bg-[#030303]/80 backdrop-blur-md border-b border-zinc-200 dark:border-white/10 text-zinc-900 dark:text-white flex justify-between items-center transition-colors duration-500">
      <a href="/" className="flex items-center gap-2 font-bold text-sm uppercase tracking-widest hover:text-green-500 transition-colors group">
...
const NextProject = ({ next }) => {
  return (
    <section className="h-screen flex items-center justify-center border-t border-zinc-200 dark:border-white/10 relative overflow-hidden bg-zinc-900 dark:bg-[#030303] group cursor-pointer" onClick={() => window.location.href = `/projects/${next.id}`}>
...
export default function ProjectDetailTemplate() {
  const [isDark, setIsDark] = useState(true); // Iniciamos en oscuro por defecto
  ...
  useEffect(() => {
    // Configuración de Lenis para esta página
    ...
    // Volver arriba al cargar (útil en React Router)
    window.scrollTo(0, 0);
  ...
  useLayoutEffect(() => {
    let ctx = gsap.context(() => {
      // Animaciones genéricas de fade in para secciones
  ...
  // Nota: En un caso real, projectData vendría de props o de un fetch usando useParams()
